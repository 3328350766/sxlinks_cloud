 ...
 */
@RestController
@RequestMapping("/miniProgram/ruleEngine/dataForward")
@Slf4j
@Api(value = "规则引擎", tags = {"数据转发"})
public class MiniDataForwardController {
	@Autowired
	...
	public Result<DataForwardItem> editItem(@RequestBody DataForwardItem item) {
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		Result<DataForwardItem> result = new Result<DataForwardItem>();
		DataForwardItem sysrole = dataForwardItemService.getById(item.getId());
