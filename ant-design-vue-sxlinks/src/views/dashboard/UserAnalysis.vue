            ...
            <a-tab-pane key="2" tab="告警日志" force-render>
              ...
              <div class="tabList">
                <template v-if="messes.length">
                  ...
                  <div class="level">
                    <p class="messesTitle">预警级别:<span>{{item.type}}级</span></p>
                    <p class="messesTitle">
                      设备名称:<span>{{ item.alarmName }}</span>
                    </p>
                    <p class="messesTitle">时间:<span>{{item.createTime  }}</span></p>
                  </div>
                  <div class="content">
...
export default {
  ...
  methods: {
    ...
    handler({ BMap, map }) {
      ...
      this.center.lng = 118.78221
      this.center.lat = 30.944371
      this.zoom = 13
    },
    projectChange(e) {
      this.getProjectList()
