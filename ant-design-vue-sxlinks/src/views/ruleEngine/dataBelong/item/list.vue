<template>
    ...
    <!--table块状结束-->
    <!-- 表单区域 -->
    <ItemModal ref="modalForm" @callback="getDataList"></ItemModal>
    </a-modal>
...
export default {
  ...
  methods: {
      ...
      // 删除
    handleDelete(id) {
      deleteProperty(`/ruleEngine/dataBelong/deleteItem`,{propertyId:id}).then((res) => {
        if (res.success) {
    ...
    //     data.propertyId=Number(data.propertyId)
     ...
     getDataList() {
        getQueryById('/ruleEngine/dataBelong/listItem', this.queryParam).then((res) => {
