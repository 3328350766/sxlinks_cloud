 ...
 * @Since：   2022-06-02
 */
public interface VideoDeviceChannelMapper extends BaseMapper<VideoDeviceChannel> {


	@Insert("INSERT INTO video_device_channel (channelId, deviceId, name, manufacture, model, owner, civilCode, block, " +
			"address, parental, parentId, safetyWay, registerWay, certNum, certifiable, errCode, secrecy, " +
			"ipAddress, port, password, PTZType, status, streamId, longitude, latitude, longitudeGcj02, latitudeGcj02, " +
			"longitudeWgs84, latitudeWgs84, hasAudio, create_time, updateTime, businessGroupId, gpsTime) " +
			"VALUES ('${channelId}', '${deviceId}', '${name}', '${manufacture}', '${model}', '${owner}', '${civilCode}', '${block}'," +
			"'${address}', ${parental}, '${parentId}', ${safetyWay}, ${registerWay}, '${certNum}', ${certifiable}, ${errCode}, '${secrecy}', " +
			"'${ipAddress}', ${port}, '${password}', ${PTZType}, ${status}, '${streamId}', ${longitude}, ${latitude}, ${longitudeGcj02}, " +
			"${latitudeGcj02}, ${longitudeWgs84}, ${latitudeWgs84}, ${hasAudio}, #{createTime,jdbcType=DATE}, '${updateTime}', '${businessGroupId}', '${gpsTime}')")
	int add(VideoDeviceChannel channel);
	...
	@Update(value = {" <script>" +
			...
			"WHERE id='${id}'"+
			" </script>"})
	int update(VideoDeviceChannel channel);
	...
	@Select(value = {" <script>" +
			...
			" </script>"})
	List<VideoDeviceChannel> queryChannels(@Param(value = "deviceId") String deviceId,@Param(value = "parentChannelId") String parentChannelId,@Param(value = "query")  String query,@Param(value = "hasSubChannel")  Boolean hasSubChannel,@Param(value = "online")  Boolean online);

	@Select("SELECT * FROM video_device_channel WHERE deviceId=#{deviceId} AND channelId=#{channelId}")
	VideoDeviceChannel queryChannel(String deviceId, String channelId);

	@Delete("DELETE FROM video_device_channel WHERE deviceId=#{deviceId}")
	int cleanChannelsByDeviceId(String deviceId);

	@Delete("DELETE FROM video_device_channel WHERE deviceId=#{deviceId} AND channelId=#{channelId}")
	int del(String deviceId, String channelId);

	@Update(value = {"UPDATE video_device_channel SET streamId=null WHERE deviceId=#{deviceId} AND channelId=#{channelId}"})
	void stopPlay(String deviceId, String channelId);

	@Update(value = {"UPDATE video_device_channel SET streamId=#{streamId} WHERE deviceId=#{deviceId} AND channelId=#{channelId}"})
...
//			" LEFT JOIN video_device de ON dc.deviceId = de.deviceId " +
//			" LEFT JOIN video_platform_gb_channel pgc on pgc.VideoDeviceChannelId = dc.id " +
//			" WHERE 1=1 " +
//			" <if test='query != null'> AND (dc.channelId LIKE '%${query}%' OR dc.name LIKE '%${query}%' OR dc.name LIKE '%${query}%')</if> " +
//			" <if test='online == true' > AND dc.status=1</if> " +
//			" <if test='online == false' > AND dc.status=0</if> " +
...
//			" <if test='catalogId == null ' >  AND dc.id not in (select VideoDeviceChannelId from platform_gb_channel where platformId=#{platformId} ) </if> " +
...
//			" ORDER BY dc.deviceId, dc.channelId ASC" +
//			" </script>"})
//	List<VideoDeviceChannelInPlatform> queryChannelByPlatformId(String platformId);


	@Select("SELECT * FROM video_device_channel WHERE channelId=#{channelId}")
	List<VideoDeviceChannel> queryChannelByChannelId( String channelId);

	@Update(value = {"UPDATE video_device_channel SET status=0 WHERE deviceId=#{deviceId} AND channelId=#{channelId}"})
	void offline(String deviceId,  String channelId);

	@Update(value = {"UPDATE video_device_channel SET status=0 WHERE deviceId=#{deviceId}"})
	...
	@Insert("<script> " +
			...
			"${item.latitudeGcj02},${item.longitudeWgs84}, ${item.latitudeWgs84}, ${item.hasAudio},#{item.createTime,jdbcType=DATE}, '${item.updateTime}', " +
			...
			"</script>")
	int batchAdd(List<VideoDeviceChannel> addChannels);

	@Update(value = {"UPDATE video_device_channel SET status=1 WHERE deviceId=#{deviceId} AND channelId=#{channelId}"})
	...
	@Update({"<script>" +
			...
			"</foreach>" +
			"</script>"})
	int batchUpdate(List<VideoDeviceChannel> updateChannels);
	...
	@Select(value = {" <script>" +
			...
			" </script>"})
	List<VideoDeviceChannel> queryChannelsByDeviceIdWithStartAndLimit(String deviceId, String parentChannelId, String query,
																 Boolean hasSubChannel, Boolean online, int start, int limit);

	@Select("SELECT * FROM video_device_channel WHERE deviceId=#{deviceId} AND status=1")
	List<VideoDeviceChannel> queryOnlineChannelsByDeviceId(String deviceId);
	...
	@Delete(value = {" <script>" +
			...
			" </script>"})
	int cleanChannelsNotInList(String deviceId, List<VideoDeviceChannel> channels);

	@Update(" update video_device_channel" +
			" set subCount = (select *" +
			"                from (select count(0)" +
	...
	@Update(value = {" <script>" +
			...
			" </script>"})
	void updatePosition(VideoDeviceChannel VideoDeviceChannel);

	@Select("SELECT * FROM video_device_channel WHERE length(trim(streamId)) > 0")
	List<VideoDeviceChannel> getAllChannelInPlay();

	@Select("select * from video_device_channel where longitude*latitude > 0 and deviceId = #{deviceId}")
	List<VideoDeviceChannel> getAllChannelWithCoordinate(String deviceId);
	...
	@Select(value = {" <script>" +
			...
			" </script>"})
	List<VideoDeviceChannel> getChannelsWithCivilCodeAndLength(String deviceId, String parentId, Integer length);

	@Select(value = {" <script>" +
			"select * " +
			"from video_device_channel " +
			"where deviceId=#{deviceId} and length(channelId)>14" +
			" </script>"})
	IPage<VideoDeviceChannel> getChannelsByCivilCodePage(IPage<VideoDeviceChannel> iPage,String deviceId);
	...
	@Select(value = {" <script>" +
			...
			" </script>"})
	List<VideoDeviceChannel> getChannelsByCivilCode(String deviceId, String parentId);
	...
	@Select("select * from video_device_channel where deviceId=#{deviceId} and civilCode not in " +
			"(select civilCode from device_channel where deviceId=#{deviceId} group by civilCode)")
	List<VideoDeviceChannel> getChannelWithoutCiviCode(String deviceId);

	@Select("select * from video_device_channel where deviceId=#{deviceId} and SUBSTRING(channelId, 11, 3)=#{typeCode}")
	List<VideoDeviceChannel> getBusinessGroups(String deviceId, String typeCode);
	...
	@Select("select dc.id, dc.channelId, dc.deviceId, dc.name, dc.manufacture,dc.model,dc.owner, pc.civilCode,dc.block, " +
			...
			" from video_device_channel dc" +
			" left join video_platform_gb_channel pgc on  dc.id = pgc.VideoDeviceChannelId" +
			" left join video_platform_catalog pc on pgc.catalogId = pc.id and pgc.platformId = pc.platformId" +
			" where pgc.platformId=#{serverGBId}")
	List<VideoDeviceChannel> queryChannelWithCatalog(String serverGBId);

	@Select("select * from video_device_channel where deviceId = #{deviceId}")
	List<VideoDeviceChannel> queryAllChannels(String deviceId);
}
