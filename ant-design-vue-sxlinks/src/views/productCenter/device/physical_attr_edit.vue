    ...
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="model" :rules="validatorRules">
            ...
            <div class="col-2">
              ...
              <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="pointType" required label="点位类型">
                <j-dict-select-tag
                  placeholder="请选择点位类型"
                  dictCode="point_type"
                  v-model="model.pointType"
                />
              </a-form-model-item>
            </div>
            <div class="col-2">
            ...
            <div class="col-2">
              <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="isBeishu" required label="开启倍数">
                <j-dict-select-tag
                  placeholder="请选择开启倍数"
                  dictCode="yn"
                  v-model="model.isBeishu"
                />
              </a-form-model-item>
              <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="倍数值" prop="beishu" required>
                  <a-input placeholder="请输入倍数值" v-model="model.beishu" type="number"/>
                </a-form-model-item>
            </div>
			<div class="col-2">
  ...
  data() {
    return {
      ...
      validatorRules: {
        ...
        pointType: [{ required: true, message: '请选择点位类型' }],
        isBeishu: [{ required: true, message: '请选择开启倍数' }],
        beishu: [{ required: true, message: '请输入倍数值' }],
      },
      url: {
        add: '/productCenter/device/addProperty',
        edit: '/productCenter/device/editProperty',
      },
      readOnly: false,
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (this.type == 'add') {
            ...
            addProperty('/productCenter/device/addProperty', this.model)
              .then((res) => {
                if (res.success) {
              ...
              .finally(() => {
                ...
                that.close()
			  	...
			  	that.$message.warning("设备属性编码已经存在，请重新输入");
			  }		
		...
		handleChangeCheckPropertyCode(){
		  ...
		  getAction(`/productCenter/device/checkPropertyCode`,{code:this.model.identifier}).then(res=>{
		    if(res.success){
