...
import com.sxlinks.modules.system.mapper.ruleEngine.AlarmMapper;
...
public class AlarmServiceImpl extends ServiceImpl<AlarmMapper, Alarm> implements IAlarmService {
    @Override
    public IPage<Alarm> custom(IPage<Alarm> iPage,Alarm alarm) {
        return baseMapper.custom(iPage,alarm);
    }

    @Override
    public Alarm getByAlarmId(Long alarmId) {
        return baseMapper.getByAlarmId(alarmId);
    }


    @Override
    public Integer del(Long alarmId) {
        return baseMapper.del(alarmId);
    }
//    @Autowired
