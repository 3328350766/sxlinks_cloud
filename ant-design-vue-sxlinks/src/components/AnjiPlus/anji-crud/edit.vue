...
export default {
  ...
  methods: {
    ...
    initDefaultSaveForm() {
      // saveForm的默认值
      let defaultSaveForm = {};
      console.log(this.option.columns,'columnscolumnscolumnscolumns');
    ...
    async queryByPrimarykey(rowData) {
      const { result, code } = await this.option.buttons.queryByPrimarykey.api(
        rowData
      );
      if (code != "200") return;
      this.showDialog = true;
      this.saveForm = this.handleDetailData(result);
    },
    // 保存前，先调用校验
