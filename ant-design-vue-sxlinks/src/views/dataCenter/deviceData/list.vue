<template>
  <a-card :bordered="false">
            ...
            <div class="list">
              ...
              <div class="bottomList">
                <div class="rol" v-for="(i, index2) in item.lsPropertyData" :key="index2">
                  ...
                  <div class="rolR">
                    <span>{{ i.value }}</span>
                    <code><span @click="issue(i,item)">下发</span>|<span @click="details(i,item)">历史</span></code>
                  </div>
                </div>
              </div>
            </div>
    ...
    <!-- 表单区域 -->
    <DeviceData ref="modalForm" :deviceCode="deviceCode"></DeviceData>
	<!--属性数据详情-->
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/dataCenter/deviceData/list',
        delete: '/dataCenter/deviceData/delete',
        deleteBatch: '/dataCenter/deviceData/deleteBatch',
        exportXlsUrl: '/dataCenter/deviceData/exportXls',
        importExcelUrl: '/dataCenter/deviceData/importExcel',
      },
      queryParam: {
        deviceName: null,
        pageNo: 1,
  ...
  methods: {
	  ...
	  issue(item,i) {
	    ...
	    this.$refs.DevicePropertyDataSet.loadData(item.identifier, i.deviceCode)
	  },
	  //设备属性数据详情
	  details(item,i) {
    ...
    confirm(i, deviceCode) {
      ...
      addFunction(`/dataCenter/deviceData/unbind`, formData).then((res) => {
        if (res.success) {
    ...
    // 查询
    searchQuery() {
      getlistFunction(`/dataCenter/deviceData/list`, this.queryParam).then((res) => {
        if (res.success) {
  ...
  // 查询网关下的所有子设备
  subEquipment(item) {
    getlistFunction(`/dataCenter/deviceData/queryAllSubDevice?deviceCode=${item.deviceCode}`).then((res) => {
      if (res.success) {
      }
    })
  },
