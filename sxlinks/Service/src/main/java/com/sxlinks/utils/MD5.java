...
//	}
	...
	public static String EncoderByMd5(String str) {
		...
		try {
			...
			//加密后的字符串
			String newstr = base64en.encode(md5.digest(str.replace("http://", "").replace("https://", "").getBytes("utf-8")));
			//return newstr.replace("/", "").replace("+", "").replace("=", "");
			rstr = newstr.replace("/", "").replace("+", "").replace("=", "");
		}catch(Exception e){
			e.printStackTrace();
		}
		return rstr;
