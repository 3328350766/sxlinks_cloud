...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/ruleEngine/alarm/list',
        delete: '/ruleEngine/alarm/delete',
        deleteBatch: '/ruleEngine/alarm/deleteBatch',
        exportXlsUrl: '/ruleEngine/alarm/exportXls',
        importExcelUrl: 'ruleEngine/alarm/importExcel',
      },
      visible: false,
  ...
  methods: {
    showAttribute(code){
      ...
      // 删除
    handleDelete(id) {
      deleteProperty(`/ruleEngine/condition/deleteItem`,{propertyId:id}).then((res) => {
        if (res.success) {
    ...
    // 新增
      handleAdd(){
        ...
        this.$refs.modalForm.getEquipmentNameList(this.deviceCode,this.queryParam.conditionShellId)
      ...
      functionEdit(item) {
        ...
        data.targetFunctionId=Number(data.targetFunctionId)
     ...
     getEnergyDataList() {
        getQueryById('/ruleEngine/condition/listItem', this.queryParam).then((res) => {
