  ...
  >
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="model" :rules="validatorRules">
		...
		<div class="col-2">
		    <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="type" required label="消警模式">
		      <j-dict-select-tag  placeholder="请选择消警模式" dictCode="cancel_mode" v-model="model.cancelMode"/>       
		    </a-form-model-item>
		    <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="duration" required label="持续时长">
		      <a-input placeholder="持续时长(秒),默认为30秒" v-model="model.duration" />
		    </a-form-model-item>
		</div>
		<div class="col-2">
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="sourceDeviceId" required label="指定设备">
          <a-select ref="select" v-model="model.sourceDeviceId" placeholder="请选择设备" style="width:250px">
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
		...
		cancelMode: [{ required: true, message: '请选择消警模式' }],
		state: [{ required: true, message: '请输入持续时长' }],
      },
      url: {
  ...
  methods: {
        ...
        if (valid) {
          ...
          if (this.type == 'add') {
			  if(this.requestCodeSuccess){
				addProperty('/ruleEngine/alarm/add', this.model)
				  .then((res) => {
					if (res.success) {
            ...
            editProperty('/ruleEngine/alarm/edit', this.model)
              .then((res) => {
                if (res.success) {
                  that.$message.success(res.message)
                  that.$parent.$parent.getEnergyDataList()
	...
	handleChangeCheckCode(){
	  ...
	  getAction(`/ruleEngine/alarm/checkCode`,{code:this.model.code}).then(res=>{
	    if(res.success){
