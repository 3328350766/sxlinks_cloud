    ...
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        <div class="col-2">
          ...
          <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="identifier" required label="功能编码">
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        add: '/productCenter/product/addFunction',
        edit: '/productCenter/product/editFunction',
      },
      readOnly: false,
    }
  },
    ...
    handleOk() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (this.type == 'add') {
            addFunction('/productCenter/product/addFunction', this.model)
              .then((res) => {
                if (res.success) {
            ...
            editProperty('/productCenter/product/editFunction', this.model)
              .then((res) => {
                if (res.success) {
                  that.$message.success(res.message)
                  that.$parent.$parent.getFunctionData()
