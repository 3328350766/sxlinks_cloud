...
export default {
  ...
  props: {
    option: {
      ...
      default: () => {
        return {
          formHeight: "800px",//this.$refs.formSearch.$el.offsetHeight
          // 详情页中输入框左边文字宽度
          labelWidth: "120px",
          // 查询区隐藏
          queryFormHide: false,
  ...
  methods: {
    ...
    async handleQueryPageList() {
      ...
      const { result, code } = await this.option.buttons.query.api(params);
      if (code != "200") return;
      this.records = result.records;
      this.total = result.total;
      console.log(result.records,'result.recordsresult.recordsresult.recordsresult.recordsresult.recordsresult.records');
    },
    // 重置
    handleResetForm() {
