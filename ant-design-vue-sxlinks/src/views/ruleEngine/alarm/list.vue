<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button @click="handleAdd" type="primary" icon="plus">创建预警</a-button>
    </div>
    ...
    <div class="app-list">
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="showModal(record)" style="margin-right:8px">设置预警子项</a>
...
export default {
  ...
  methods: {
      // 删除
    handleDelete(id) {
      deleteProperty(`/ruleEngine/alarm/delete?id=${id}`).then((res) => {
        if (res.success) {
    ...
    // 新增
     ...
     getEnergyDataList() {
        getQueryById('/ruleEngine/alarm/list', this.queryParam).then((res) => {
