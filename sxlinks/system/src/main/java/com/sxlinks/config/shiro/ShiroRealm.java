...
import com.sxlinks.common.api.CommonAPI;
...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        LoginUser loginUser = JSON.parseObject(JSON.toJSONString(this.checkUserTokenIsEffect(token)),LoginUser.class);
        //LoginUser sysUser = JSON.parseObject(JSON.toJSONString(sysBaseAPI.getUserByName(username)),LoginUser.class);
    ...
    public LoginUser checkUserTokenIsEffect(String token) throws AuthenticationException {
        ...
        LoginUser loginUser = JSON.parseObject(JSON.toJSONString(commonAPI.getUserByName(username)),LoginUser.class);
        if (loginUser == null) {
    ...
    public boolean jwtTokenRefresh(String token, String userName, String passWord) {
        ...
        if (oConvertUtils.isNotEmpty(cacheToken)) {
            ...
            //update-begin--Author:baba  Date:20191005  for：解决每次请求，都重写redis中 token缓存问题
//			else {
//				// 设置超时时间
//				redisUtil.set(CommonConstant.PREFIX_USER_TOKEN + token, cacheToken);
//				redisUtil.expire(CommonConstant.PREFIX_USER_TOKEN + token, JwtUtil.EXPIRE_TIME / 1000);
//			}
            //update-end--Author:baba  Date:20191005   for：解决每次请求，都重写redis中 token缓存问题
            return true;
        }
        return false;
    }
