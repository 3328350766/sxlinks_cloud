...
export default {
  ...
  methods: {
    showAttribute(code){
      ...
      // 删除
    handleDelete(id) {
      deleteProperty(`/ruleEngine/alarm/deleteItem`,{propertyId:id}).then((res) => {
        if (res.success) {
    ...
    // 新增
      handleAdd(){
		...
		//加载属性列表
        this.$refs.modalForm.getPropertyList(this.deviceCode,this.queryParam.alarmId)
     ...
     getEnergyDataList() {
        getQueryById('/ruleEngine/alarm/listItem', this.queryParam).then((res) => {
