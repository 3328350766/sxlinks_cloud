 ...
 */
@RestController
@RequestMapping("/miniProgram/productCenter/device")
@Slf4j
...
public class MiniDeviceController {
	...
	@RequestMapping(value = "/runtimeById", method = RequestMethod.GET)
	@ApiOperation(value = "设备运行状态", httpMethod = "GET", response = DataResult.class,tags = "设备管理")
	public DataResult<List<DeviceRtResDto>> runtimeById(Long id) {
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		if (StringUtils.isEmpty(id)) {
			return DataResult.fail(BCErrorEnum.INVALID_PARAM);
		}
		Device device=deviceService.getById(id);
		List<DeviceRtResDto> result = deviceService.queryRtByDevCode(device.getDeviceCode());
		return DataResult.ok(result);
	}
	...
	@RequestMapping(value = "/queryPropertyDataHistoryDataById", method = RequestMethod.POST)
	@ApiOperation(value = "查看设备具体属性历史数据列表-根据id", httpMethod = "POST", response = DataResult.class, notes = "设备管理")
	public DataResult<PageResDto<DeviceRtHistoryResDto>>
	rtItemById(@RequestBody PageReqDto<DeviceRtItemReqDto> searchPage) {
		PageResDto<DeviceRtHistoryResDto> pageResult = deviceDataService.searchReportItem(searchPage);
		return DataResult.ok(pageResult);
	}
	/**
