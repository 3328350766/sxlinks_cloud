<template>
  <div id="DeviceTree" style="width: 100%;height: 100%; background-color: #FFFFFF; overflow: auto">
    <el-container>
      <el-header style="height:18px">设备列表</el-header>
...
<script>
import { getAction,getListProperty } from '@/api/product'
      ...
      loadNode: function(node, resolve){
        ...
        if (node.level === 0) {
          if (this.device) {
            ...
            getAction("/video/device/list",{pageNo:1,pageSize:50}).then((data)=>{
              if (data.result.records.length > 0) {
                let nodeList = []
                let result=data.result.records
                for (let i = 0; i < result.length; i++) {
                  let node = {
                    name: result[i].name || result[i].deviceId,
                    isLeaf: false,
                    id: result[i].deviceId,
                    type: result[i].online,
          ...
          let channelArray = []
          getListProperty("/video/channel/tree/"+node.data.userData.deviceId,{pageNo:1,pageSize:50}).then((res)=>{
            console.log(catalogData)
            let catalogData=res.result.records
