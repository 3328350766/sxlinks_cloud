<template>
    ...
    <!--table块状结束-->
    <!-- 表单区域 -->
    <Product-modal ref="modalForm" @callback="getEnergyDataList"></Product-modal>
...
export default {
  ...
  methods: {
    showAttribute(code){
      ...
      // 删除
    handleDelete(id) {
      deleteProperty(`/ruleEngine/dataForward/deleteItem`,{propertyId:id}).then((res) => {
        if (res.success) {
    ...
    // 新增
      handleAdd(){
		...
		this.$refs.modalForm.model.dataForwardId=this.queryParam.dataForwardId
        this.$refs.modalForm.type = 'add'
		//加载属性列表
        this.$refs.modalForm.getDeviceList()
     ...
     getEnergyDataList() {
        getQueryById('/ruleEngine/dataForward/listItem', this.queryParam).then((res) => {
	...
	// 复制
		copy(item) {
		  ...
		  this.$refs.DeviceCopy.title = '复制 新子项信息(默认为禁用状态)'
