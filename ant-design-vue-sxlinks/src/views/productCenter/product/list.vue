        ...
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="6">
            <a-form-item label="产品编码">
              <a-input placeholder="请输入产品编码" v-model="queryParam.productCode" style="width:180px"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="6">
            <a-form-item label="产品名称">
              <a-input placeholder="请输入产品名称" v-model="queryParam.productName" style="width:180px"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="6">
		    ...
		    <a-list-item slot="renderItem" slot-scope="item, index">
		      <a-card :hoverable="true">
                ...
                <div class="contant">
                  ...
                  <div class="list">
                    <p>状态</p>
                    <code><i :class="item.status==0?'coler':''"></i>{{item.status==0?'禁用':'启用'}}</code>
                  </div>
		            ...
		            <a-dropdown>
		              ...
		              <a-menu slot="overlay">
		                <a-menu-item>
						  ...
						  <a-popconfirm title="确定删除吗?" @confirm="() => delClick(item.id)">
						    <a>删除</a>
						  </a-popconfirm>
		                </a-menu-item>
		                <a-menu-item>
		                  
						  <a-popconfirm title="确定激活吗?" @confirm="() => enable(item.id)">
						    <a>激活</a>
						  </a-popconfirm>
		                </a-menu-item>
		                <a-menu-item>
		                  
						  <a-popconfirm title="确定禁用吗?" @confirm="() => disable(item.id)">
						    <a>禁用</a>
						  </a-popconfirm>
		                </a-menu-item> 
		              </a-menu>
...
import { getlistFunction, getQueryById, getListProperty, deleteProperty,getAction } from '@/api/product'
...
export default {
  ...
  methods: {
    ...
    addattrBut() {
	  ...
	  this.$refs.PhysicalAttrEdit.model = {}
      //将产品编码传入
    ...
    delClick(id) {
      deleteProperty(`/productCenter/product/delete?id=${id}`).then((res) => {
		...
		enable(id) {
		  getAction(`/productCenter/product/enable`,{id:id}).then((res) => {
		    if (res.success) {
		      this.$message.success(res.message)
		      this.searchQuery()
		    } else {
		...
		disable(id) {
		  getAction(`/productCenter/product/disable?id=${id}`).then((res) => {
		    if (res.success) {
		      this.$message.success(res.message)
		      this.searchQuery()
		    } else {
