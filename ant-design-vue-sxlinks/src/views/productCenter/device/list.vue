<template>
  <a-card :bordered="false">
    <div class="identification">
      <p v-for="(item, index) in identification" :key="index">
        <i></i>{{ item.title }}<span>{{ item.value }}</span>
      </p>
    </div>
    <!-- 查询区域 -->
        ...
        <a-row :gutter="24">
          <a-col :md="6" :sm="8">
            <a-form-item label="设备编码">
              <a-input placeholder="请输入设备编码" v-model="queryParam.deviceCode" style="width:180px"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="8">
            <a-form-item label="设备名称">
              <a-input placeholder="请输入设备名称" v-model="queryParam.deviceName" style="width:180px"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="8">
            <a-form-item label="节点类型">
              <j-dict-select-tag v-model="queryParam.nodeType" placeholder="请选择节点" dictCode="node_type"  width="180px"/>
            </a-form-item>
          </a-col>
    ...
    <div class="app-list">
      <a-row>
        <a-col :span="5">
        <!-- <div class="allEquipment">所有产品</div> -->
        <a-list size="small" bordered :data-source="dataList">
           <a-list-item v-for="(item,index) in dataList" :key="index" :class="index==active?'active':''" @click="listChang(item,index)">{{ item.productName }}</a-list-item>
        </a-list>
        </a-col>
        <a-col :span="18"  style="margin-left:10px">
          ...
          >
            <span slot="action" slot-scope="text, record">
              <a href="javascript:;" @click="detailsDevClick(record.id, record.deviceCode)">运行状态</a>
              <a-divider type="vertical" />
				<a @click="editClick(record)" style="margin:0 8px">编辑</a>
				<a-divider type="vertical" />
				<a @click="copy(record)" style="margin:0 8px">复制</a>
				<a-divider type="vertical" />			
              <a-popconfirm title="确定删除吗?" @confirm="() => delClick(record.id)">
                      <a>删除</a>
              </a-popconfirm>
	...
	<DeviceCopy ref="DeviceCopy" @ok="modalFormOk"></DeviceCopy>
...
export default {
  ...
  methods: {
    ...
    // 产品左侧列表点击事件
    listChang(item,index){
      this.queryParam.productCode=item.productCode 
      this.active=index
      this.searchQuery()
    },
    //下发
    ...
    editClick(item) {
	  ...
	  this.$refs.DeviceEdit.getProjectById(data.projectId)
    },
		// 复制
		copy(item) {
		  let data = JSON.parse(JSON.stringify(item))
			
		  this.$refs.DeviceCopy.visible = true
		  this.$refs.DeviceCopy.title = '复制 新设备信息'
		  this.$refs.DeviceCopy.type = 'copy'
			this.$refs.DeviceCopy.empty()
		  this.$refs.DeviceCopy.model = data
			this.$refs.DeviceCopy.getProjectById(data.projectId)
		},
    // 删除list数据
    delClick(id) {
    ...
    getProduct(){
      ...
      getQueryById('/productCenter/product/queryAll').then((res)=>{
        ...
        that.dataList=[...obj,...res.result]
        that.queryParam.productCode=that.dataList[0].productCode
        that.searchQuery()
      })
    },
    searchReset(){
