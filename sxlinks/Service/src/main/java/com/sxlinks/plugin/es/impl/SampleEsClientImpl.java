package com.sxlinks.plugin.es.impl;
...
import com.sxlinks.plugin.es.utils.JsonUtil;
import com.sxlinks.utils.BeanUtil;
import com.sxlinks.utils.DateUtil;
import com.sxlinks.utils.JSONProvider;
import com.sxlinks.utils.Md5Utils;
...
public class SampleEsClientImpl implements SampleEsClient {
    ...
    @Value("${sxlinks.es.shards:5}")
    Integer shards;
    @Value("${sxlinks.es.replicas:1}")
    Integer replicas;
    @Value("${sxlinks.es.refresh:1s}")
    String refresh;
    @Autowired
    ...
    public boolean batchInsertRequest(List list, String routing)  {
        ...
        try {
            for (int i = 0; i < list.size(); i++) {
                ...
                EsInsertDataBo data = (EsInsertDataBo) list.get(i);

                IndexRequest request1 = null;
                String id= com.sxlinks.utils.StringUtils.getNewOrderNo();
                Map<String, Object> map = BeanUtil.beanToMap(data.getEsMessage());
                map.remove("saveTimestamp");
                map.remove("timestamp");
                //map.put("saveTimestamp", DateUtil.getTimeMs(data.getEsMessage().getSaveTimestamp())); //解决批量保存的时间类型
                //map.put("timestamp", DateUtil.getTimeMs(data.getEsMessage().getTimestamp()));
                map.put("saveTimestamp", data.getEsMessage().getSaveTimestamp().getTime()); //解决批量保存的时间类型
                map.put("timestamp", data.getEsMessage().getTimestamp().getTime());

                if (StringUtils.isNotEmpty(routing)) {
                    request1 = new IndexRequest(data.getIndexName()).routing(routing).source(map, XContentType.JSON).id(id);
                } else {
                    request1 = new IndexRequest(data.getIndexName()).source(map, XContentType.JSON).id(id);
                }
            ...
            if (bulk.status().getStatus() == 200) {
                ...
                log.error("批量创建索引{}文档失败",bulk.buildFailureMessage());
            }
        }catch(Exception e){
            e.printStackTrace();
        }
