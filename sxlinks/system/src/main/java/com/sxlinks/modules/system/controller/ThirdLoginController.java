...
import com.sxlinks.modules.system.entity.SysThirdAccount;
import com.sxlinks.modules.system.entity.SysUser;
import com.sxlinks.modules.system.model.ThirdLoginModel;
import com.sxlinks.modules.system.service.ISysThirdAccountService;
...
public class ThirdLoginController {
    ...
    public String loginThird(@PathVariable("source") String source, AuthCallback callback,ModelMap modelMap) {
        ...
        if(response.getCode()==2000) {
        	...
        	//判断有没有这个人
			//update-begin-author:baba date:20201118 for:修改成查询第三方账户表
        	...
        	// 生成token
			//update-begin-author:baba date:20201118 for:从第三方登录查询是否存在用户id，不存在绑定手机号
			...
			//update-end-author:baba date:20201118 for:从第三方登录查询是否存在用户id，不存在绑定手机号
		//update-begin--Author:baba  Date:20200729 for：接口在签名校验失败时返回失败的标识码 issues#1441--------------------
        }else{
			modelMap.addAttribute("token", "登录失败");
		}
		//update-end--Author:baba  Date:20200729 for：接口在签名校验失败时返回失败的标识码 issues#1441--------------------
	...
	public Result<String> thirdUserCreate(@RequestBody ThirdLoginModel model) {
		...
		//创建新账号
		//update-begin-author:baba date:20201118 for:修改成从第三方登录查出来的user_id，在查询用户表尽行token
		...
		if(oConvertUtils.isNotEmpty(user.getSysUserId())){
			...
			// 生成token
			String token = saveToken(sysUser);
			//update-end-author:baba date:20201118 for:修改成从第三方登录查出来的user_id，在查询用户表尽行token
			res.setResult(token);
			res.setSuccess(true);
		}
		return res;
	...
	public Result<JSONObject> getThirdLoginUser(@PathVariable("token") String token,@PathVariable("thirdType") String thirdType) throws Exception {
		...
		//update-begin-author:baba date:20201118 for:如果真实姓名和头像不存在就取第三方登录的
		...
		//update-end-author:baba date:20201118 for:如果真实姓名和头像不存在就取第三方登录的
