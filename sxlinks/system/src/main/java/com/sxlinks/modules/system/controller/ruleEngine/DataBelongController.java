...
import com.sxlinks.modules.system.service.ruleEngine.*;
 ...
 */
@RestController
@RequestMapping("/ruleEngine/dataBelong")
@Slf4j
@Api(value = "数据归类引擎", tags = {"数据归类"})
public class DataBelongController {
	@Autowired
	...
	public Result<?> delete(@RequestParam(name="id",required=true) String id) {
		belongProductService.remove(new QueryWrapper<BelongProduct>().eq("belong_id",id));
		belongTypeItemService.remove(new QueryWrapper<BelongTypeItem>().eq("belong_id",id));
