  ...
  >
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="model" :rules="validatorRules" :form="model">
        ...
        <div class="col-2">
          <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="productCode" required label="产品归属">
            <a-select v-model="model.productCode" placeholder="请输入产品归属" :disabled="type == 'add' ? false : true">
              <a-select-option :value="item.productCode" v-for="(item, index) in productSelect" :key="index">
                {{ item.productName }}
              </a-select-option>
            </a-select>
        ...
        <div class="col-2">
          ...
          <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="deviceCode" required label="设备编码	">
            <a-input placeholder="请输入设备编码" v-model="model.deviceCode" :disabled="type == 'add' ? false : true"  @blur="handleChangeCheckCode"/>
          </a-form-model-item>
        </div>
        <div class="col-2">
          <a-form-model-item
            ...
            label="协议类型"
          >
            <j-dict-select-tag placeholder="请选择协议类型" dictCode="protocol_type" v-model="model.protocolType" />
          </a-form-model-item>
          <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="networkType" required label="网络类型">
            <j-dict-select-tag placeholder="请选择网络类型" dictCode="network_type" v-model="model.networkType" />
          </a-form-model-item>
        </div>
        ...
        <div class="col-2">
          <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="nodeType" required label="节点类型">
          ...
          <a-form-model-item
            ...
            required
            label="网关类型"
            v-if="model.nodeType == 'GATEWAY'"
          >
            <j-dict-select-tag placeholder="请选择网关类型" dictCode="gateway_type" v-model="model.gatewayType" />
          </a-form-model-item>
          <a-form-model-item
            :labelCol="labelCol"
            :wrapperCol="wrapperCol"
            prop="gwDevCode"
            label="网关列表"
            v-else-if="model.nodeType == 'SUB'"
          >
            <a-select
              style="width: 100%"
              @change="handleSelectChange"
              v-model="model.gwDevCode"
              placeholder="请选择网关列表"
            >
              <a-select-option v-for="(item, index) in gatewayOption" :key="index" :value="item.deviceCode">
                {{ item.deviceName }}
              </a-select-option>
            </a-select>
          </a-form-model-item>
          <a-form-model-item v-else></a-form-model-item>
        </div>
        <div class="col-2">
  ...
  data() {
    return {
      ...
      validatorRules: {
        ...
        gwDevCode: [{ required: true, message: '请输入网关列表', trigger: 'change' }],
        nodeType: [{ required: true, message: '请输入节点类型', trigger: 'change' }],
        networkType: [{ required: true, message: '请输入网络类型', trigger: 'change' }],
        protocolType: [{ required: true, message: '请输入协议类型', trigger: 'change' }],
        productCode: [{ required: true, message: '请输入产品编码', trigger: 'change' }],
        gatewayType: [{ required: true, message: '请输入网关类型', trigger: 'change' }],
        ...
        if (valid) {
          ...
          if (this.type == 'add') {
			  if(this.requestCodeSuccess){
				addProduct(`/productCenter/device/add`, model).then((res) => {
				  if (res.success) {
            ...
            editProperty('/productCenter/device/edit', model)
              .then((res) => {
                if (res.success) {
